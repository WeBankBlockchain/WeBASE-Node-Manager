...
public interface TbStatMapper {
     ...
     */
    @Delete({
            "DELETE FROM tb_stat AS tb USING (SELECT max(block_number) AS maxBlock FROM tb_stat WHERE group_id = #{groupId}) AS tmp WHERE tb.group_id = #{groupId} AND tb.block_number <= tmp.maxBlock - ${blockRetainMax};"})
    Integer remove(@Param("groupId") Integer groupId, @Param("blockRetainMax") BigInteger blockRetainMax);

    @Select({"select id,group_id,block_cycle,tps,block_number,block_size,stat_timestamp,create_time,modify_time from tb_stat where group_id = #{groupId}  and stat_timestamp between #{startTimestamp} and #{endTimestamp} order by id"})
    ...
    @Select({"select id,group_id,block_cycle,tps,block_number,block_size,stat_timestamp,create_time,modify_time from tb_stat where id = (select max(id) from tb_stat where group_id = #{groupId})"})
    TbStat getMaxByGroupId(@Param("groupId") Integer groupId);

    @Select({ "select id,group_id,block_cycle,tps,block_number,block_size,stat_timestamp,create_time,modify_time from tb_stat where  group_id = #{groupId} and block_number = #{blockNumber}"})
