...
public interface TbConfigMapper {
    ...
    @Select({
        "select id,config_name,config_type,config_value,create_time,modify_time from tb_config where config_value = #{configValue,jdbcType=VARCHAR} order by id desc"
    })
    ...
    @Select({
            "select id,config_name,config_type,config_value,create_time,modify_time from tb_config where config_type = #{type,jdbcType=SMALLINT} order by id desc"
    })
     ...
     */
    @Delete({
        "delete from tb_config where id = #{id,jdbcType=INTEGER}"
    })
    ...
    int insertSelective(@Param("record") TbConfig record);
     ...
     */
    @Select({
        "select id, config_name, config_type, config_value, create_time, modify_time from tb_config where id = #{id,jdbcType=INTEGER}"
    })
    @Results({
    ...
    TbConfig selectByPrimaryKey(@Param("id") Integer id);
    ...
    int updateByPrimaryKeySelective(@Param("record") TbConfig record);
