...
public interface TbHostMapper {
    ...
    @Update({
            " update tb_host set status=#{newStatus},modify_time=#{modifyTime} where id = #{hostId} and status != #{newStatus}"
    })
    int updateChainStatus(@Param("hostId") int hostId, @Param("modifyTime") Date modifyTime, @Param("newStatus") byte newStatus );
