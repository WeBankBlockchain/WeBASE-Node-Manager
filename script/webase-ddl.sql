...
CREATE TABLE IF NOT EXISTS tb_alert_rule (
  ...
  enable tinyint(1) DEFAULT 0 NOT NULL COMMENT '是否启用规则',
  ...
  is_all_user tinyint(1) DEFAULT 0 NOT NULL COMMENT '是否选中所有用户',
...
CREATE TABLE IF NOT EXISTS tb_mail_server_config (
  ...
  authentication tinyint(1) DEFAULT 1 NOT NULL COMMENT '是否开启验证',
  starttls_enable tinyint(1) DEFAULT 1 NOT NULL COMMENT '如支持，是否优先选用STARTTLS',
  starttls_required tinyint(1) DEFAULT 0 NOT NULL COMMENT '是否必须使用STARTTLS',
  ...
  socket_factory_fallback tinyint(1) DEFAULT 0 NOT NULL COMMENT '是否启用SSL的fallback',
  status tinyint(1) DEFAULT 0 NOT NULL COMMENT '邮件服务器是否已配置完成，0初始，1完成',
